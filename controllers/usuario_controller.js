...
const getAllUsers = (req, res) => {
    db.query("SELECT * FROM usuarios")
      .then((usuarios) => {res.status(200).json(usuarios[0]);})
...
const postUser = async (req, res) => {
  ...
  db.query(
    ...
    .then((respuesta) => {res.status(201).send("Usuario registrado con éxito en la DB");})
    .catch((error) => {res.status(500).send(error);});
};
const logIn = (req, res) => {
  let user = req.body;
  db.query(
    ...
    .then(async (userLogin) => {
      ...
      } else {res.status(401).send("Contraseña no válida");}
    })
    .catch((error) => {res.status(500).send("Email o usuario no registrado");});
};
module.exports = {
    logIn,
    getAllUsers,
