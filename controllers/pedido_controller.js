    ...
    db.query(
      ...
      .then(async (respuesta) => {
        ...
        insertDetalle(detalle, idPedido)
          .then((respuesta) => {return upDateTotal(respuesta, idPedido);})
          .catch((error) => {console.log(error + " error de promesas");});
...
const getPedidoById = (req, res) => {
  ...
  .then((respuesta) => {res.json(respuesta[0])})
...
const deletePedidoUsuario = (req, res) => {  
    let idPedido = req.body.idPedido;
      ...
      .then((respuesta) => {res.status(200).send('Pedido ' + idPedido + ' Borrado')})
      .catch((error) => {res.status(500).send(error)})
}
function getPrecioPlato(idPlato) {
  return new Promise(function (resolve, reject) {
    db.query("SELECT precio FROM platos WHERE id = ?", {
      ...
      .catch((error) => {
        reject("plato inexistente" + error);
      });
  });
};
function upDateTotal(total, idPedido) {
    db.query("UPDATE pedidos SET precio_total = ? WHERE id = ? ", {replacements: [total, idPedido],})
      .then((respuesta) => {})
      .catch((error) => {console.log(error + "fallo el update");})
      .catch((error) => {reject(error + " calcular total");});
  };
  ...
  db.query('SELECT id_estado FROM pedidos WHERE id = ?',
    ...
    .then((respuesta) => {
        if((respuesta[0].id_estado === estadoNuevo)){
            res.status(401).send('Estado no se puede cambiar')
        } else {
            db.query("UPDATE pedidos SET id_estado = ?, hora = ? WHERE id = ?", {replacements: [estadoNuevo, hora, pedido],})
                .then((respuesta) => {
                  res.json({estadoActual: estadoNuevo,respuesta: respuesta[0],dateTime: hora,usuario: usuario.id,});
                })
